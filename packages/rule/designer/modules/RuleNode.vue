<template>
  ...
  <div v-else-if="node.type == 'gateway'"
      ...
      :class="{ active: isActive() }"
      :style="{ top: node.y + 'px', left: node.x + 'px', content: node.nodeName,
    		cursor: currentTool.type == 'drag' ? 'move' : (currentTool.type == 'connection' ? 'crosshair' : 
		...
		methods: {
			registerNode () {//创建新组件时调用创建新节点
				...
				that.plumb.draggable(that.node.id, {
					...
					grid: ruleConfig.defaultStyle.alignGridPX,
					drag: function(e) {
						if (ruleConfig.defaultStyle.isOpenAuxiliaryLine) {
